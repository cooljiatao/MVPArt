...
import me.jiatao.funny.mvp.ui.adapter.BeautyAdapter;
...
public class FragmentPhotoBeautyPresenter extends BasePresenter {
    private RxErrorHandler mErrorHandler;
    private List<PhotoBeautyInfo> mBeautyPhotos = new ArrayList<>();
    ...
    public FragmentPhotoBeautyPresenter(AppComponent appComponent) {
    ...
    public void requestPhotoBeauty(final Message msg) {
        ...
        if (mAdapter == null) {
            mAdapter = new BeautyAdapter(mBeautyPhotos);
            msg.what = 0;
            msg.obj = mAdapter;
        ...
        addSubscrebe(mModel.getBeautyPhotoList(lastUserId, isEvictCache)
                ...
                .subscribe(new ErrorHandleSubscriber<List<PhotoBeautyInfo>>(mErrorHandler) {
                    @Override
                    public void onNext(List<PhotoBeautyInfo> photoBeautyInfos) {
                        ++index;
                        lastUserId = index * 20;

                        if (pullToRefresh) mBeautyPhotos.clear();//如果是上拉刷新则清空列表

                        preEndIndex = mBeautyPhotos.size();//更新之前列表总长度,用于确定加载更多的起始位置
                        mBeautyPhotos.addAll(photoBeautyInfos);
